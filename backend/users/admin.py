...
#     )
    ...
    # In users/admin.py, replace your two CustomUserAdmin classes with this one:

class CustomUserAdmin(UserAdmin):
    # Your original settings
    model = CustomUser
    list_display = (
        'email', 'name', 'phone', 'user_type', 'is_staff',
        'is_active', 'gender', 'birth_date', 'date_joined', 'last_login'
    )
    list_filter = (
        'is_staff', 'is_active', 'gender', 'user_type', 'date_joined'
    )
    search_fields = ('email', 'name', 'phone')
    ordering = ('-date_joined',)  # This line fixes the startup error
    ...
    add_fieldsets = (
        (None, {
            'classes': ('wide',),
            # CHANGE 'password' TO 'password1' IN THIS LIST
            'fields': (
                'email', 'name', 'password1', 'password2',
                'phone', 'user_type', 'bio', 'gender', 'birth_date', 'profile_pic',
                'is_staff', 'is_active', 'is_superuser'
            )
        }),
    )

    # The debugging method we added
    def add_view(self, request, form_url="", extra_context=None):
        if request.method == "POST":
            form = self.get_form(request, obj=None)(request.POST, request.FILES)
